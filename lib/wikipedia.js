...
const USER_AGENT = 'MasterMindWikiQuiz/1.0 (https://github.com/acatarinasj/Master-Mind)';
...
async function fetchRandomSummary() {
  let res;
  try {
    res = await fetch(RANDOM_SUMMARY_URL, {
      headers: { 'User-Agent': USER_AGENT, Accept: 'application/json' },
    });
  } catch (err) {
    console.error(`Pedido à Wikipédia falhou: ${err.message}`);
    return null;
  }

  if (res.status === 429 || res.status === 503) {
    console.warn(`Wikipédia devolveu ${res.status}, a aguardar antes de tentar novamente...`);
    await sleep(RATE_LIMIT_BACKOFF_MS);
    return null;
  }

  if (!res.ok) {
    console.error(`Wikipédia devolveu estado inesperado: ${res.status} ${res.statusText}`);
    return null;
  }
